...
function createKv() {
  // automaticDeserialization defaults to true in @upstash/redis, which means
  // kv.get() would already return a parsed object for any value that looks
  // like JSON — but every route in this codebase does
  // JSON.parse(await kv.get(key)), expecting a raw string back (matching
  // what kv.set(key, JSON.stringify(x)) stored). Without this flag, that
  // second JSON.parse() throws on the already-parsed object ("Unexpected
  // token o in JSON"), which crashed /api/profile/me (and would eventually
  // hit /api/leaderboard, /api/ghost, /api/avatar too) with an uncaught 500
  // the moment a real profile existed to parse. Disabling it here keeps
  // kv.get() returning raw strings everywhere, matching what the rest of
  // the app already assumes.
  if (url && token) return new Redis({ url, token, automaticDeserialization: false });
